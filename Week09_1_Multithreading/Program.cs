...
namespace Week09_1_Multithreading
{
    internal class Program
    {
        ...
        {
            Console.WriteLine($"Inside the method {MethodBase.GetCurrentMethod().Name}");
            Console.WriteLine($"TPL Method Managed thread id: {Thread.CurrentThread.ManagedThreadId}");
            ...
            {
                Console.WriteLine($"For Loop Method Managed thread id: {Thread.CurrentThread.ManagedThreadId}");
        ...
        private static void UsePLINQ()
        {
            ...
            Console.WriteLine("Calling AsParallel");
            ...
            Console.WriteLine("Calling AsParallel with ForceParallelism");

            foreach (var num in numbers.Where(n => n > -1).AsParallel().WithExecutionMode(ParallelExecutionMode.ForceParallelism))
            {
                Console.WriteLine(num);
            }
            ...
            Console.WriteLine("Calling AsParallel with AsOrdered AND ForceParallelism");
            foreach (var num in numbers.Where(n => n > -1).AsParallel().AsOrdered().WithExecutionMode(ParallelExecutionMode.ForceParallelism))
            {
                Console.WriteLine(num);
            }
